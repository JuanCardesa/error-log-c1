...
it('conserva intervalos negativos y agrupa varios cloze en una misma nota', () => {
  const plan = reconcile({ cards: [card(), card({ cardId: 11, ord: 1 })], notes: [note()], reviews: { '10': [review()], '11': [review({ id: REVIEW + 1, ease: 3 })] }, missingNoteIds: [] }, EMPTY_ANKI_DATASET, NOW);
  expect(plan.cards).toHaveLength(2);
  expect(plan.notes).toHaveLength(1);
  expect(plan.reviews[0]).toMatchObject({ interval: -60, reviewDate: toIsoDate(new Date(REVIEW)) });
});
it('ignora manual, rescheduled y entradas sin botón real', () => {
  const plan = reconcile({ cards: [card()], notes: [note()], reviews: { '10': [review({ type: 4 }), review({ type: 5 }), review({ ease: 0 }), review({ ease: 5 }), review({ type: -1 })] }, missingNoteIds: [] }, EMPTY_ANKI_DATASET, NOW);
...
it('incluye repasos importados anteriores al último conocido y es idempotente', () => {
  ...
  const plan = reconcile(snapshot, current, NOW);
  expect(plan.newReviews).toBe(1);
  expect(reconcile(snapshot, { ...plan, sync: null }, NOW).newReviews).toBe(0);
...
it('rechaza notas ausentes y colisiones de identificadores', () => {
  expect(() => reconcile({ cards: [card()], notes: [], reviews: {}, missingNoteIds: [] }, EMPTY_ANKI_DATASET, NOW)).toThrow('Falta la nota');
  expect(() => reconcile({ cards: [card()], notes: [note()], reviews: { '10': [review(), review()] }, missingNoteIds: [] }, EMPTY_ANKI_DATASET, NOW)).toThrow('repetido');
  expect(() => reconcile({ cards: [card()], notes: [note()], reviews: { '10': [review({ id: Number.MAX_SAFE_INTEGER })] }, missingNoteIds: [] }, EMPTY_ANKI_DATASET, NOW)).toThrow('inválido');
});
it('permite cartas sin repasos y conserva las notas declaradas ausentes', () => {
  expect(reconcile({ cards: [card()], notes: [note()], reviews: {}, missingNoteIds: [99] }, EMPTY_ANKI_DATASET, NOW)).toMatchObject({ reviews: [], missingNoteIds: [99] });
});
