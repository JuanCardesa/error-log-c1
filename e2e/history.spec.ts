...
test('permite abrir sesiones antiguas y volver a la primera pagina', async ({ page }) => {
  ...
  await expect(pages.getByRole('link', { name: /Más antiguas/ })).toBeVisible();
  await pages.getByRole('link', { name: /Más antiguas/ }).click();
  await expect(page).toHaveURL(/p=2/);

  // Un numero fuera de rango se limita a la ultima pagina con datos.
  await page.goto('/registrar?p=999999');
  await expect(pages.getByRole('link', { name: /Más antiguas/ })).toHaveCount(0);
  ...
  await expect(pages.getByRole('link', { name: /Más recientes/ })).toHaveCount(0);
});
